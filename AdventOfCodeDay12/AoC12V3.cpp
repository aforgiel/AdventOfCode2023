...
Record::CountUnfoldedArrangements(int copies)
{
	...
	*tmp = '\0';
	...
	explore = new char[size + 2 + 1];

	// Case 1: Nominal [XXX]
	explore[0] = '\0';
	strncpy(explore+1, row, size);
	explore[size+1] = '\0';
	V(N) = RecursiveCountV2(explore+1, 0, 0, damages, size, minSize);

	// Case 2: Begin [?XXX]
	explore[0] = '?';
	V(B) = RecursiveCountV2(explore, 0, 0, damages, size + 1, minSize);

	// Case 3: Mid [?XXX?]
	explore[size+1] = '?';
	explore[size + 2] = '\0';
	V(M) = RecursiveCountV2(explore, 0, 0, damages, size + 2, minSize);

	// Case 3: End [XXX?]
	explore[0] = '\0';
	V(E) = RecursiveCountV2(explore+1, 0, 0, damages, size + 1, minSize);
	...
	if (V(M) > MAX(V(B),V(E)))
		sensitivity++;
	printf(" N: %zd, B: %zd, M: %zd, E: %zd = > sensitivity: % d\n", V(N), V(B), V(M), V(E), sensitivity );
	if (sensitivity < 2)
	{
		validation = RecursiveProbability(Input::Start, 0, 4, input);
		printf("\tvalidation = %zd (%zd) %s\n", validation, result, validation == result ? "OK" : "WRONG");
	}
	delete explore;
